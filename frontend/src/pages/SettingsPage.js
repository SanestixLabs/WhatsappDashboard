...
export default function SettingsPage() {
  const [active, setActive] = useState('canned');
      ...
      <div style={{width:'230px', borderRight:'1px solid rgba(255,255,255,0.06)', padding:'24px 0', flexShrink:0, overflowY:'auto'}}>
        ...
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActive(tab.id)}
            ...
            <div style={{color: active===tab.id ? '#00d4b8' : '#536471', display:'flex', alignItems:'center', transition:'color 0.15s'}}>
              {tab.icon}
            </div>
            <div>
              <div style={{fontSize:'13px', fontWeight: active===tab.id ? '600' : '400', color: active===tab.id ? '#e9edef' : '#8696a0', transition:'color 0.15s'}}>{tab.label}</div>
